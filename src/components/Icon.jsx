/* ── Shared vector icon set (Lucide-style) ────────────────────────────────────
   Crisp at any size. Icons inherit the surrounding text colour (currentColor)
   and default to 1em so the existing font-size CSS controls their size.
   Usage: <Icon name="car" />  or  <Icon name="phone" size={24} />            */
...
const STROKE = {
  ...
  van: <><path d="M3 17h1m12 0h4v-5l-2-5H8m-5 10V7a2 2 0 0 1 2-2h3v12m-7 0h3" /><circle cx="7.5" cy="17.5" r="1.5" /><circle cx="17.5" cy="17.5" r="1.5" /></>,
  ...
  road: <><path d="M4 19 8 5M20 19 16 5M12 5v2M12 11v2M12 17v2" /></>,
  ...
  tag: <><path d="M12 2H2v10l9.29 9.29a1 1 0 0 0 1.42 0l7.29-7.29a1 1 0 0 0 0-1.42Z" /><path d="M7 7h.01" /></>,
  ...
  snow: <><line x1="12" y1="2" x2="12" y2="22" /><path d="m20 7-8 5-8-5M20 17l-8-5-8 5M5.5 4.5 12 8l6.5-3.5M5.5 19.5 12 16l6.5 3.5" /></>,
...
export default function Icon({ name, size = '1em', className = '', strokeWidth = 1.9 }) {
  if (FILL[name]) {
    return (
...
/* Morocco flag — a real flag image beats an emoji for "Made in Morocco". */
export function MoroccoFlag({ size = '1em', className = '' }) {
  return (
    <svg className={className} viewBox="0 0 24 16" width={size} height={size} aria-hidden focusable="false">
      <rect width="24" height="16" rx="2" fill="#c1272d" />
      <path d="M12 4.4l1.05 3.23h3.4l-2.75 2 1.05 3.23L12 12.86l-2.75 2 1.05-3.23-2.75-2h3.4z"
        fill="none" stroke="#006233" strokeWidth="0.9" strokeLinejoin="round" />
    </svg>
  )
}
